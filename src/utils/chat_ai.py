...
# Cost cap: max API calls per stream session
...
- If asked about your capabilities, you can only chat — you can't control the stream.
...
class ChatAI:
    ...
    def ask(self, username: str, message: str) -> str | None:
        ...
        This is the ONLY public method. It's fully sandboxed:
        - No tool access
        - No file access
        - Read-only context
        - Rate limited
        ...
        try:
            ...
            response = client.messages.create(
                ...
                system=system,
                messages=[
                    {"role": "user", "content": f"{username} asks: {message}"}
                ],
            )
            ...
            logger.info(f"Chat AI [{self._call_count}/{MAX_CALLS_PER_SESSION}]: {username}: {message[:50]} -> {result[:50]}")
